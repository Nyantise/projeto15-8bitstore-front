...
export default function MyOrderPage(){
    ...
    function totalValue(){
        let tValue = 0
        MyOrder.map(item => tValue += item.price*item.quantity)
        return tValue
    }

    function order(){

        const newMyOrder = MyOrder.map(item => {return {productId: item.gameid, quantity: item.quantity}})

        const URL = apiURL+"MyOrder/"+user.id
        const body = {
            products: newMyOrder
        }
        const config = {
